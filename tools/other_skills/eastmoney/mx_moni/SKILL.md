...
## 基础信息
- 版本：v1.0.4
- 描述：妙想提供的股票模拟组合管理系统，支持持仓查询、买卖操作、撤单、委托查询等
- 所属分类：模拟交易
- 适用场景：模拟炒股、策略验证、持仓查询、买卖操作
...
## 目录层信息
- 关键参数：自然语言操作指令（必填）
- 核心目标：通过自然语言指令操作模拟组合（持仓查询、买卖、撤单等）

## 工具列表
### 工具1：mx_moni_operation
- 功能：妙想模拟组合管理，支持持仓查询、买卖操作、撤单、委托查询等
- 调用入口：mx_moni_operation
- 参数约束：
  - query: str（必填）- 自然语言操作指令，如"我的持仓"、"买入 600519 价格 1700 数量 100 股"

## 使用指南

### 适用场景
- 用户需要**查询模拟账户**的持仓、资金、委托、历史成交时
- 用户需要进行**模拟交易操作**（买入、卖出、撤单）时
- 用户需要**验证交易策略**时

### 典型调用流程
1. **查询类操作**：mx_moni_operation("我的持仓") / mx_moni_operation("我的资金") / mx_moni_operation("我的委托")
2. **买入操作**：mx_moni_operation("买入 600519 价格 1700 数量 100 股") 或 mx_moni_operation("市价买入 600519 100 股")
3. **卖出操作**：mx_moni_operation("卖出 600519 价格 1750 数量 100 股")
4. **撤单操作**：mx_moni_operation("撤单 260854300000078983") 或 mx_moni_operation("一键撤单")

### 注意事项
- 这是**模拟交易**，不涉及真实资金
- 买入/卖出的股票代码为6位数字，数量必须为100的整数倍
- 限价委托需提供价格，市价委托无需价格
- 撤单需要提供委托编号，可先通过"我的委托"查询获取
- 非交易时间（9:30-11:30/13:00-15:00）无法下单
