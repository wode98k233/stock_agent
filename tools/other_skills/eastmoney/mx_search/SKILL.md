...
## 基础信息
- 版本：v1.0.5
- 描述：基于东方财富妙想搜索能力，金融场景信源智能筛选，获取新闻、公告、研报等资讯
- 所属分类：资讯搜索
- 适用场景：金融资讯搜索、新闻查询、研报获取、公告查询
...
## 目录层信息
- 关键参数：自然语言搜索问句（必填）
- 核心目标：搜索金融相关资讯（新闻、研报、公告等），避免参考非权威过时信息

## 工具列表
### 工具1：mx_search_news
- 功能：妙想资讯搜索，搜索金融相关资讯（新闻、研报、公告等）
- 调用入口：mx_search_news
- 参数约束：
  - query: str（必填）- 自然语言搜索问句，如"贵州茅台最新研报"

## 使用指南

### 适用场景
- 需要获取**时效性资讯**（最新新闻、公告、研报）时使用本工具，而非 sentiment_analysis
- 需要了解**特定事件影响**（如"美联储加息对A股影响"）时
- 需要查询**交易规则、政策解读**等非常识信息时
- 本工具侧重**信息检索**，sentiment_analysis 侧重**情感分析**，两者可配合使用

### 典型调用流程
1. **新闻+情感联合分析**：mx_search_news(搜索资讯) → 将搜索结果传给 sentiment_analysis 的 analyze_news_sentiment 做情感分析
2. **个股资讯查询**：mx_search_news("贵州茅台最新研报")
3. **板块/行业新闻**：mx_search_news("人工智能板块近期新闻")
4. **宏观事件分析**：mx_search_news("美联储加息对A股影响分析")

### 注意事项
- query 参数使用自然语言，越具体搜索结果越精准
- 本工具返回的是资讯原文/摘要，不含情感判断，如需情感分析请配合 sentiment_analysis 使用
- 本工具基于东方财富权威信源，可避免参考非权威或过时信息
