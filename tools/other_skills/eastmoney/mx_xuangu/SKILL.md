...
## 基础信息
- 版本：v1.0.4
- 描述：基于行情指标、财务指标等条件智能筛选股票，支持行业/板块查询和成分股查询
- 所属分类：智能选股
- 适用场景：条件选股、行业板块筛选、指数成分股查询
...
## 目录层信息
- 关键参数：自然语言选股条件（必填）
- 核心目标：根据自然语言选股条件筛选满足条件的股票

## 工具列表
### 工具1：mx_xuangu_filter
- 功能：妙想智能选股，根据自然语言条件筛选股票
- 调用入口：mx_xuangu_filter
- 参数约束：
  - query: str（必填）- 自然语言选股条件，如"今日涨幅大于2%的A股"

## 使用指南

### 适用场景
- 需要根据**行情指标条件**（涨跌幅、成交量、市盈率等）筛选股票时
- 需要根据**财务指标条件**（ROE、净利润增长率、股息率等）筛选股票时
- 需要查询**指定行业/板块**内满足条件的股票时
- 本工具是**条件选股**，stock_query 的 get_board_stocks 是**获取板块全部成分股**，两者用途不同

### 典型调用流程
1. **简单条件选股**：mx_xuangu_filter("市盈率小于20的银行股")
2. **组合条件选股**：mx_xuangu_filter("价格小于20元 市盈率小于20 涨幅大于1% A股")
3. **行业/板块内选股**：mx_xuangu_filter("新能源板块市盈率小于30的股票")
4. **选股+分析联动**：mx_xuangu_filter(筛选股票) → 对筛选结果逐个调用 technical_analysis 做技术分析

### 注意事项
- query 参数使用自然语言描述选股条件，支持多条件组合
- 筛选结果可能为空（条件太严格），此时应放宽条件重试
- 本工具返回的是股票列表，不含深度分析数据，需配合其他技能做后续分析
