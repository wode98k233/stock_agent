...
# mx_zixuan 妙想自选管理skill

## 基础信息
- 版本：v1.0.0
- 描述：基于东方财富通行证账户数据及行情底层数据，支持自然语言查询、添加、删除自选股
- 所属分类：自选股管理
- 适用场景：自选股查询、添加自选股、删除自选股
...
## 目录层信息
- 关键参数：自然语言管理指令（必填）
- 核心目标：通过自然语言指令管理自选股（查询、添加、删除）

## 工具列表
### 工具1：mx_zixuan_manage
- 功能：妙想自选股管理，支持查询、添加、删除自选股
- 调用入口：mx_zixuan_manage
- 参数约束：
  - query: str（必填）- 自然语言管理指令，如"查询我的自选股列表"、"把贵州茅台添加到我的自选股列表"
...
### 典型调用流程
1. **查询自选股**：mx_zixuan_manage("查询我的自选股列表") 或 mx_zixuan_manage("我的自选")
2. **添加自选股**：mx_zixuan_manage("把贵州茅台添加到我的自选股列表") 或 mx_zixuan_manage("加入自选 比亚迪")
3. **删除自选股**：mx_zixuan_manage("把贵州茅台从我的自选股列表删除") 或 mx_zixuan_manage("删除自选 万科A")

### 注意事项
- 添加/删除操作前建议先查询确认当前自选股列表
- 股票名称或代码均可使用，6位数字代码成功率更高
- 添加已存在的股票或删除不存在的股票会返回错误
