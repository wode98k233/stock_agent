...
## 工具列表
### 工具1：iwc_market_query
- 功能：获取股票、ETF、指数等实时价格、涨跌幅、成交量、主力资金流向、技术指标等行情数据
- 调用入口：iwc_market_query
  - query: str（必填）- 自然语言查询问句，如"同花顺最新价格"、"上证指数行情"、"主力资金流向"

## 使用指南
- 适用场景：需要查询股票、ETF、指数等行情数据时使用（价格、涨跌幅、成交量、资金流向、技术指标）
- 典型调用：iwc_market_query("同花顺最新价格")、iwc_market_query("上证指数行情")、iwc_market_query("MACD金叉")
