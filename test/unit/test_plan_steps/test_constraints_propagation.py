...
def test_executor_prompt_includes_constraints_label():
    ...
    async def mock_ainvoke(input_dict, config=None):
        test_executor_prompt_includes_constraints_label._captured = input_dict.get("messages", [])
        return {"messages": [MagicMock(content="寒武纪股价: 150元")]}

    mock_react_agent.ainvoke = mock_ainvoke

    with patch("utils.llm_factory.get_llm", return_value=mock_llm), \
         patch("agents.plan.executor.create_react_agent", return_value=mock_react_agent), \
         patch("agents.utils._confirm_step_execution", return_value=True), \
         patch("agents.executor_callbacks.ExecutionState"), \
         patch("agents.executor_callbacks.ExecutionStateCallback"), \
         patch("utils.llm_factory.TokenTracker"), \
    ...
    msgs = test_executor_prompt_includes_constraints_label._captured
    assert msgs is not None, "executor should have been invoked with messages"
    ...
    assert "[用户约束]" in all_content, \
        f"executor prompt should contain [用户约束] label. Got:\n{all_content[:800]}"
    assert "只查询寒武纪相关数据" in all_content, \
        f"executor prompt should contain user_constraints text. Got:\n{all_content[:800]}"
...
def test_executor_prompt_no_constraints_no_label():
    ...
    async def mock_ainvoke(input_dict, config=None):
        test_executor_prompt_no_constraints_no_label._captured = input_dict.get("messages", [])
        return {"messages": [MagicMock(content="股价: 1500元")]}
    ...
    msgs = test_executor_prompt_no_constraints_no_label._captured
    all_content = ""
    for msg in msgs:
        if isinstance(msg, tuple) and len(msg) == 2:
            all_content += str(msg[1]) + "\n"
...
def test_replanner_prompt_includes_constraints_label():
    ...
    def mock_llm_json_with_retry(llm, messages, logger, label=""):
    ...
    with patch("agents.plan.replanner.get_llm", return_value=MagicMock()), \
         patch("agents.plan.replanner.llm_json_with_retry", side_effect=mock_llm_json_with_retry), \
