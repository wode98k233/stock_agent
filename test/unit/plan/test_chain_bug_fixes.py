...
def test_executor_budget_exceeded_no_await_crash():
    ...
    with patch('agents.shared.exception_utils.handle_budget_exceeded', return_value={"response": "预算超限"}):
        result = asyncio.run(
...
def test_replanner_budget_exceeded_no_await_crash():
    ...
    from agents.shared.exception_utils import handle_budget_exceeded
    with patch('agents.plan.node.replanner.handle_budget_exceeded', return_value={"response": "预算超限"}):
        result = asyncio.run(
...
def test_executor_continue_returns_not_empty_dict():
    ...
    with patch('agents.shared.exception_utils.handle_budget_exceeded', return_value={"_user_approved_overrun": True}):
        with patch('agents.plan.node.executor.run_react_subgraph', new_callable=AsyncMock, return_value={"tool_results": [], "final_result": "完成"}):
            result = asyncio.run(
                _run_executor_step(state, ctx)
            )
    # 应该返回执行结果，而非空 dict
...
def test_replanner_continue_returns_current_step():
    """Bug C: handle_budget_exceeded 返回 _user_approved_overrun=True 时，replanner 返回 current_step"""
    from utils.budget import BudgetExceeded
    ...
    ctx = _make_mock_ctx()
    ctx.budget.check = MagicMock(side_effect=BudgetExceeded(reason="time", current=601, limit=600))
    ctx.budget.set_exempt_window = MagicMock()

    with patch('agents.plan.node.replanner.handle_budget_exceeded', return_value={"_user_approved_overrun": True}):
        result = asyncio.run(
            _run_replanner_step(state, ctx)
        )
    assert isinstance(result, dict)
...
def test_executor_loop_checks_budget_before_invoke():
    ...
    with patch('agents.plan.node.executor.run_react_subgraph', new_callable=AsyncMock, return_value={"tool_results": [], "final_result": "完成"}):
        with patch('agents.shared.exception_utils.handle_budget_exceeded', return_value={"response": "预算超限"}) as mock_handle:
            result = asyncio.run(
                _run_executor_step(state, ctx)
            )
...
async def _run_executor_step(state, ctx):
    ...
    with patch('agents.utils._confirm_step_execution', return_value=True):
        with patch('agents.utils._generate_step_summary', new_callable=AsyncMock, return_value="步骤总结"):
            with patch('agents.plan.node.executor.SkillPromptBuilder.build_tools_detail_prompt', return_value="工具详情"):
...
async def _run_replanner_step(state, ctx):
    ...
    with patch('agents.plan.node.replanner.check_termination', return_value=(False, None)):
        with patch('agents.plan.node.replanner.SkillPromptBuilder.build_catalog_prompt', return_value="技能目录"):
            with patch('agents.plan.node.replanner.llm_json_with_retry', return_value={"action": "continue"}):
