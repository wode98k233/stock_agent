...
def test_stream_and_collect_returns_enhanced_response_from_dashboard():
    """dashboard 增强后的 response 应被返回，而非 replanner 的原始 response"""
    events = [
        {"executor": {"past_steps": [("Step 1", "r1")], "current_step": 1}},
        {"replanner": {"response": "原始报告"}},
        {"report_enhance": {}},
        {"dashboard": {"response": "原始报告\n\n📊 仪表盘"}},
    ]
    app = FakeAstreamApp(events)
    inputs = {
        "input": "test",
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        test_stream_and_collect_returns_enhanced_response_from_dashboard,
    ]
    passed = 0
    failed = 0
    for t in tests:
