...
def test_exempt_window_active_after_create():
    ...
    from utils.budget import BudgetExemptWindow
...
def test_executor_success():
    ...
    with patch("agents.plan.node.executor.run_react_subgraph", new_callable=AsyncMock, return_value={"tool_results": [], "final_result": "测试结果数据，包含足够的内容用于验证执行成功"}), \
         patch("utils.llm_factory.get_llm", return_value=MagicMock()), \
         patch("agents.plan.node.executor.SkillPromptBuilder.build_tools_detail_prompt", return_value="工具详情"), \
...
def test_executor_uses_plan_step_skill_first():
    """E1b: executor 优先使用 plan step 指定的 skill 的工具，不再合并 selected_skills。"""
    from agents.plan.node.executor import execute_step

    ctx = _make_mock_ctx()
    plan = [{"step": 1, "skill": "test_skill", "instruction": "执行测试指令", "purpose": "验证工具合并"}]
    state = _make_base_state(plan)
    state["selected_skills"] = ["mx_data", "mx_search"]

    def fake_get_tools(skill_name):
        assert isinstance(skill_name, str), f"get_tools 只能接收单个技能名，实际收到: {type(skill_name).__name__}"
        return [SimpleNamespace(name=f"{skill_name}_tool", description=f"{skill_name} 工具")]

    captured_tool_names = []

    async def fake_run_react_subgraph(**kwargs):
        captured_tool_names.extend([tool.name for tool in kwargs["tools"]])
        return {
            "tool_results": [],
            "final_result": "执行成功，selected_skills 工具已经按单个技能名合并，返回足够长的结果用于通过异常结果检查。",
        }

    ctx.skill_registry.get_tools = MagicMock(side_effect=fake_get_tools)

    with patch("agents.plan.node.executor.run_react_subgraph", new_callable=AsyncMock, side_effect=fake_run_react_subgraph), \
         patch("utils.llm_factory.get_llm", return_value=MagicMock()), \
         patch("agents.plan.node.executor.SkillPromptBuilder.build_tools_detail_prompt", return_value="工具详情"), \
         patch("agents.utils._generate_step_summary", new_callable=AsyncMock, return_value={}), \
         patch("agents.utils._confirm_step_execution", return_value=True), \
         patch("agents.plan.analyze.extract_step_info", return_value={
             "skill_name": "test_skill", "instruction": "执行测试指令",
             "step_num": 1, "purpose": "验证工具合并"
         }):
        result = asyncio.run(execute_step(state, ctx))

    # 只加载 plan step 指定的 test_skill 的工具，不加载 selected_skills
    called_skills = [call.args[0] for call in ctx.skill_registry.get_tools.call_args_list]
    assert called_skills == ["test_skill"]
    assert captured_tool_names == ["test_skill_tool"]
    assert result["step_results"][0]["status"] == "success"


def test_executor_fallback_to_selected_skills_when_plan_skill_empty():
    """E1b-fallback: 当 plan step 的 skill 无工具时，才加载 selected_skills 的工具。"""
    from agents.plan.node.executor import execute_step

    ctx = _make_mock_ctx()
    plan = [{"step": 1, "skill": "empty_skill", "instruction": "执行测试指令", "purpose": "验证工具合并"}]
    state = _make_base_state(plan)
    state["selected_skills"] = ["mx_data", "mx_search"]

    def fake_get_tools(skill_name):
        assert isinstance(skill_name, str), f"get_tools 只能接收单个技能名，实际收到: {type(skill_name).__name__}"
        if skill_name == "empty_skill":
            return []
        return [SimpleNamespace(name=f"{skill_name}_tool", description=f"{skill_name} 工具")]
    ...
    with patch("agents.plan.node.executor.run_react_subgraph", new_callable=AsyncMock, side_effect=fake_run_react_subgraph), \
         ...
         patch("agents.utils._confirm_step_execution", return_value=True), \
         patch("agents.plan.analyze.extract_step_info", return_value={
             "skill_name": "empty_skill", "instruction": "执行测试指令",
             "step_num": 1, "purpose": "验证工具合并"
         }):
        result = asyncio.run(execute_step(state, ctx))

    # plan step 的 empty_skill 无工具，fallback 到 selected_skills
    called_skills = [call.args[0] for call in ctx.skill_registry.get_tools.call_args_list]
    assert called_skills == ["empty_skill", "mx_data", "mx_search"]
    assert captured_tool_names == ["mx_data_tool", "mx_search_tool"]
    assert result["step_results"][0]["status"] == "success"


def test_unified_executor_uses_plan_step_skill_first():
    """E1c: UnifiedPlan 优先使用 plan step 指定的 skill 的工具。"""
    ...
    async def fake_run_react_subgraph(**kwargs):
        ...
        return {
            "tool_results": [],
            "final_result": "统一执行成功，selected_skills 工具已经按单个技能名合并，返回足够长的结果用于通过长度检查。",
        }

    ctx.skill_registry.get_tools = MagicMock(side_effect=fake_get_tools)

    with patch("agents.plan.node.unified_executor.run_react_subgraph", new_callable=AsyncMock, side_effect=fake_run_react_subgraph), \
         patch("utils.llm_factory.get_llm", return_value=MagicMock()):
        result = asyncio.run(unified_execute_step(state, ctx))

    # 只加载 plan step 指定的 test_skill 的工具
    called_skills = [call.args[0] for call in ctx.skill_registry.get_tools.call_args_list]
    assert called_skills == ["test_skill"]
    assert captured_tool_names == ["test_skill_tool"]
    assert result["response"].startswith("统一执行成功")


def test_executor_partial_completion():
    """E2: 部分完成 → step_results status=partial（通过模拟异常触发）"""
    from agents.plan.node.executor import execute_step, _build_step_result
    ...
    with patch("agents.plan.node.executor.create_react_agent", return_value=mock_agent), \
         patch("utils.llm_factory.get_llm", return_value=MagicMock()), \
         patch("agents.plan.node.executor.SkillPromptBuilder.build_tools_detail_prompt", return_value="工具详情"), \
         ...
         patch("agents.plan.node.executor.generate_partial_summary", new_callable=AsyncMock, return_value="部分完成的摘要"), \
         patch("agents.utils._confirm_step_execution", return_value=True), \
         patch("agents.plan.analyze.extract_step_info", return_value={
             "skill_name": "test_skill", "instruction": "测试指令",
             "step_num": 1, "purpose": "测试目的"
...
def test_replanner_all_completed():
    ...
    with patch("agents.plan.node.replanner.get_llm", return_value=MagicMock()), \
         ...
         patch("agents.plan.node.replanner.check_termination", return_value=(True, "all_steps_completed")):
...
def test_replanner_continue():
    ...
    with patch("agents.plan.node.replanner.get_llm", return_value=MagicMock()), \
         patch("agents.plan.node.replanner.llm_json_with_retry", return_value=llm_result), \
         patch("agents.plan.node.replanner.SkillPromptBuilder.build_catalog_prompt", return_value="catalog"), \
...
def test_replanner_budget_awareness():
    ...
    with patch("agents.plan.node.replanner.get_llm", return_value=MagicMock()), \
         patch("agents.plan.node.replanner.llm_json_with_retry", side_effect=capture_llm_json), \
         patch("agents.plan.node.replanner.SkillPromptBuilder.build_catalog_prompt", return_value="catalog"), \
         patch("agents.plan.node.replanner.check_termination", return_value=(False, None)):
        asyncio.run(replan_step(state, ctx))
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        test_executor_merges_selected_skills_one_by_one,
        test_unified_executor_merges_selected_skills_one_by_one,
