...
def test_text_similarity():
    """验证文本相似度计算"""
    from agents.plan.analyze import text_similarity
    ...
    # 部分相似
    sim = text_similarity("茅台股价分析", "茅台股价查询")
    assert 0.3 < sim < 0.9
...
def test_datasource_manager_mark_failed():
    ...
    # 重置状态（_fail_counts 需要预初始化，register_source 时会设置为 0）
    DataSourceManager._sources = [source]
    DataSourceManager._fail_counts = {"test_source": 0}
    DataSourceManager._disabled_at = {}
    DataSourceManager._current_source_index = 0

    # 标记失败
    DataSourceManager.mark_source_failed("test_source")
    assert DataSourceManager._fail_counts["test_source"] == 1

    # 继续失败
    DataSourceManager.mark_source_failed("test_source")
    assert DataSourceManager._fail_counts["test_source"] == 2
...
def test_datasource_manager_auto_disable():
    ...
    # 连续失败达到阈值
    max_fails = Config.DATASOURCE_MAX_FAILS
    for _ in range(max_fails):
        DataSourceManager.mark_source_failed("test_source")

    # 应该被禁用
    assert source.enabled == False
    assert "test_source" in DataSourceManager._disabled_at
...
def test_datasource_manager_reset():
    ...
    # 重置状态
    DataSourceManager._sources = [source]
    DataSourceManager._fail_counts = {"test_source": 5}
    DataSourceManager._disabled_at = {"test_source": time.time()}

    # 手动重置
    DataSourceManager.reset_source("test_source")

    assert source.enabled == True
    assert DataSourceManager._fail_counts["test_source"] == 0
    assert "test_source" not in DataSourceManager._disabled_at
...
def test_datasource_manager_switch():
    ...
    DataSourceManager._fail_counts = {}
    DataSourceManager._disabled_at = {}
    DataSourceManager._current_source_index = 0
...
def test_datasource_manager_status():
    ...
    # 重置状态
    DataSourceManager._sources = [source]
    DataSourceManager._fail_counts = {"test_source": 2}
    DataSourceManager._disabled_at = {}
    ...
    assert status["test_source"]["fail_count"] == 2
