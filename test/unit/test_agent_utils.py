"""
Test: Agent 工具函数
验证: agents/plan/analyze.py - is_abnormal_result, error_fingerprint, text_similarity
"""
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '..'))

from agents.plan.analyze import is_abnormal_result, error_fingerprint, text_similarity
...
def test_text_similarity_identical():
    print("\n" + "=" * 60)
    print("TC-AgentUtils-03: 完全相同字符串")
    print("=" * 60)

    sim = text_similarity("搜索电池板块新闻", "搜索电池板块新闻")
    print(f"  完全相同: {sim}")
...
def test_text_similarity_empty():
    print("\n" + "=" * 60)
    print("TC-AgentUtils-04: 空字符串处理")
    print("=" * 60)

    assert text_similarity("", "") == 0.0
    assert text_similarity("abc", "") == 0.0
    assert text_similarity("", "xyz") == 0.0
    print("[OK] 空字符串返回 0.0")


def test_text_similarity_threshold_07():
    print("\n" + "=" * 60)
    print("TC-AgentUtils-05: 目的相似度阈值 0.7")
    print("=" * 60)

    pairs = [
        ("搜索电池板块新闻", "搜索电池板块新闻", True),
        ("宁德时代走势分析", "宁德时代走势分析", True),
        ("搜索电池板块新闻", "搜索电池板块新文", True),
        ("电池板块资讯", "电池板块资讯", True),
...
def test_text_similarity_threshold_085():
    print("\n" + "=" * 60)
    print("TC-AgentUtils-06: 指令重复度阈值 0.85")
    print("=" * 60)

    pairs = [
        ("搜索电池板块新闻", "搜索电池板块新闻", True),
        ("搜索电池板块资讯", "搜索电池板块资讯", True),
        ("电池板块分析", "电池板块分析", True),
        ("搜索电池板块新闻", "搜索电池板块资讯", False),
        ("搜索电池新闻", "查询券商板块行情", False),
    ]
    ...
    for a, b, expected in pairs:
        ...
        result = sim > 0.85
        status = "通过" if result == expected else "失败"
        print(f"  [{status}] '{a}' vs '{b}': {sim:.3f} {'>' if result else '<='} 0.85")
        assert result == expected, f"期望 {expected}, 得到 {result}"

    print("[OK] 阈值 0.85 判断正确")
...
def test_replan_dedup_logic():
    ...
    is_dup_purpose = any(
        text_similarity(new_step_purpose, p) > 0.7
        for p in completed_purposes
    )
    is_dup_instr = any(
        text_similarity(new_step_instruction, i) > 0.85
        for i in completed_instructions
    )
    is_dup_pending = any(
        text_similarity(new_step_instruction, p) > 0.7
        for p in pending_instructions
    )

    print(f"  目的相似: {is_dup_purpose} (相似度 {text_similarity(new_step_purpose, completed_purposes[0]):.3f})")
...
def test_replan_all_unique():
    ...
    sim = text_similarity(new_step_purpose, completed_purposes[0])
    is_dup = sim > 0.7
...
if __name__ == "__main__":
    try:
        ...
        test_text_similarity_threshold_07()
        test_text_similarity_threshold_085()
