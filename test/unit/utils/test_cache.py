...
def test_next_trading_open_time_monday():
    ...
    with patch("utils.cache.market._cal_get_next_trading_date") as mock_next:
        mock_next.return_value = datetime(2026, 5, 11).date()  # 周一
...
def _setup_memory_db():
    """创建 in-memory SQLite 并初始化 cache_kv 表"""
...
def test_cache_set_and_get():
    ...
    with patch("utils.cache.core._get_cache_db") as mock_get_db:
...
def test_cache_expired_entry():
    ...
    with patch("utils.cache.core._get_cache_db") as mock_get_db:
        ...
        conn.execute(
            "INSERT INTO cache_kv (cache_key, data, created_at, expire_at) VALUES (?, ?, ?, ?)",
            ("cache_news:old_key", json.dumps({"old": True}), created_at, expire_at)
        )
        conn.commit()
